#!/usr/bin/env python3
"""Tower-Waker — Wake-on-LAN gate + manual LAN wake page for tower.

Two jobs, one tiny stdlib server on fastpi (host network, :9002 — 9001 is
Beefy-Waker):

1. Traefik forwardAuth gate at **/gate** — called before proxying any tower-bound
   route: tower up -> 200 (proxy through); tower asleep -> send a WoL magic packet
   and return 503 + an auto-refreshing "waking up" page.

2. Manual LAN wake page at **/** — open in a browser to wake tower on demand:
   fires the magic packet, shows a countdown to the typical boot time, and polls
   /status in the background until tower answers, then shows "up and running".
   Reachable LAN-only (host port, not public) e.g. via a Traefik route
   `https://tower-wol.fastpi.homelab/`.

Endpoints:
  GET  /          state + manual wake page (HTML); no auto-wake on load
  GET  /wol       same page but auto-fires WoL on load (countdown)
  GET  /status    JSON {"up": true|false}   (TCP-probes tower)
  POST /wake      fire the WoL magic packet; JSON {"sent": true|false}
  GET  /gate      forwardAuth gate (200 up / 503 + page down); ?port= override
  GET  /history   boot/sleep timeline — INERT ON TOWER, see below

/history is inherited from Beefy-Waker and cannot work here: it shells out to
`journalctl --list-boots` over SSH, and Unraid is not systemd and has no journal.
The compose file deliberately mounts no key, so the endpoint returns "history key
not installed" and the page's history panel stays empty. Left in place rather
than stripped so the two wakers remain diffable.

Config via env (see .env):
  WAKER_LISTEN_PORT  port to listen on (host network)        default 9001 (set 9002)
  WAKER_BIND         address to bind; set to the LAN IP      default 0.0.0.0
  TARGET_MAC          MAC to wake                             required
  TARGET_BROADCAST    broadcast address for the magic packet  default 255.255.255.255
  TARGET_PROBE_HOST   host to TCP-probe for "is it up"        required
  TARGET_PROBE_PORT   default probe port                      default 22 (set 8080)
  WAKE_COUNTDOWN     page countdown seconds (typical boot)   default 60 (set 120)
  TARGET_SSH_USER    /history SSH user                       default buntu (unused)
  TARGET_HISTORY_SINCE  /history cutoff epoch                default 0 (unused)
...
MAC = os.environ.get("TARGET_MAC", "")
BROADCAST = os.environ.get("TARGET_BROADCAST", "255.255.255.255")
PROBE_HOST = os.environ.get("TARGET_PROBE_HOST", "")
PROBE_PORT = int(os.environ.get("TARGET_PROBE_PORT", "22"))
COUNTDOWN = int(os.environ.get("WAKE_COUNTDOWN", "60"))
SSH_USER = os.environ.get("TARGET_SSH_USER", "buntu")  # for /history journal fetch
...
HISTORY_SINCE = int(os.environ.get("TARGET_HISTORY_SINCE", "0"))
...
function showUp(){ show('done'); document.title = 'tower is up'; }
function tick(){
  left -= 1;
...
// --- collapsed "tower history" panel: lazy-fetch on first open ---
...
hist.addEventListener('toggle', async () => {
  ...
  try{
    ...
    body.innerHTML = '<p class="muted">History loads once tower is awake. '
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _history(self):
        # Fetch tower's boot/sleep history via the read-only forced-command key.
        # The forced command (in tower's authorized_keys) only ever returns
        ...
        try:
            ...
            if r.returncode == 0 and out.startswith("["):
                ...
                self._send(503, "application/json",
                           json.dumps({"error": "tower unreachable"}).encode())
        except Exception as e:
    ...
    def _gate(self):
        # host is hard-wired to tower. `?port=` lets a sibling middleware probe a
        # specific tower service port for precise readiness; it is range-checked and
        ...
        try:
            send_wol(MAC, BROADCAST)
            self.log_message("tower down (%s:%d) - sent WoL to %s via %s",
...
def main():
    missing = [k for k, v in (("TARGET_MAC", MAC),
                              ("TARGET_PROBE_HOST", PROBE_HOST)) if not v]
    if missing:
    ...
    print("tower-waker listening on %s:%d - page=/ gate=/gate, wake %s via %s, probe %s:%d"
